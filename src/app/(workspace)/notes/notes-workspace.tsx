...
/* eslint-disable @next/next/no-img-element -- authenticated file previews use short-lived blob URLs. */
...
import { createClient as createBrowserSupabaseClient } from "@/persistence/supabase/browser";
            ...
            <div className="notes-new-menu" role="menu">
              ...
              {props.isAdmin ? (
                <CreateChoice
                  ...
                  icon={<FileText />}
                  label="Google Note"
                  onClick={() => setCreateMode("google_doc")}
                />
              ) : null}
              <CreateChoice
                icon={<Upload />}
        ...
        <div className="notes-folder-canvas" ref={canvasRef}>
          ...
          {folders.map((folder, index) => {
            ...
            const position = positions[folder.id] ?? {
              x: (defaultIndex % 5) * 128,
              y: Math.floor(defaultIndex / 5) * 96,
            };
            return (
              <div
                className="notes-folder-slot"
                key={folder.id}
              ...
              >
                ...
                <div className="notes-folder-actions">
                  <EditIconButton
                    label={`Renomear pasta ${folder.name}`}
                    onClick={() => setRenameFolder(folder)}
                  />
                  {props.isAdmin ? (
                    <DeleteIconButton
                      label={`Eliminar pasta ${folder.name}`}
                      onClick={() =>
                        setDeleteTarget({ id: folder.id, kind: "folder", title: folder.name })
                      }
                    />
                  ) : null}
                </div>
      ...
      <section aria-labelledby="notes-documents-title" className="notes-documents-section">
        ...
        <div className="notes-document-grid">
            ...
            <div className="notes-document-row" key={item.id}>
              <button className="notes-document" onClick={() => open(item)} type="button">
                ...
                <span>
                  ...
                  <small>
                    {formatKind(item)} · {formatDate(item.updatedAt)}
                  </small>
                </span>
              </button>
              {props.isAdmin ? (
          ...
          {visibleItems.length === 0 ? (
            <p className="notes-inline-empty">No documents in this folder.</p>
          ) : null}
        </div>
      </section>
...
function RenameFolderDialog({
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(payload?.error ?? "Não foi possível renomear.");
        return;
      }
      onSaved(name.trim());
    } catch {
...
function DeleteDialog({
  ...
  async function remove() {
    ...
    setError(null);
    const url =
      target.kind === "folder" ? `/api/notes/folders/${target.id}` : `/api/notes/${target.id}`;
    const response = await fetch(url, { method: "DELETE" });
    const payload = await response.json().catch(() => null);
    if (!response.ok) {
      setError(payload?.error ?? "Não foi possível eliminar.");
      setDeleting(false);
      return;
    }
    const folder =
      target.kind === "folder" && selectedFolderId === target.id ? null : selectedFolderId;
  ...
  return (
    <div aria-modal="true" className="notes-dialog-backdrop" role="dialog">
      <div className="notes-delete-dialog">
        <DialogHeader
          onClose={onClose}
          title={target.kind === "folder" ? "Eliminar pasta?" : "Eliminar documento?"}
        />
        <p>
        ...
        <p>
          {target.kind === "folder"
            ? "Os documentos ficam disponíveis em All documents."
            : "Esta ação não pode ser anulada no OS."}
        </p>
        {error ? <p className="notes-form-error">{error}</p> : null}
        <div className="notes-delete-actions">
          ...
          <button className="button-danger" disabled={deleting} onClick={remove} type="button">
  ...
  async function uploadFile(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (file instanceof globalThis.File && file.size > NOTE_UPLOAD_MAX_BYTES) {
        setError("O ficheiro deve ter no máximo 10 MB.");
        return;
      }
      if (!(file instanceof globalThis.File)) {
        setError("Escolhe um ficheiro.");
        return;
      }
      ...
      const prepareResponse = await fetch("/api/notes/upload", {
      ...
      if (!prepareResponse.ok) {
        setError(prepared.error ?? "Não foi possível preparar a importação.");
        return;
      }
      ...
      if (uploadError) {
        setError(`Não foi possível enviar o ficheiro: ${uploadError.message}`);
        return;
      }
      const completeResponse = await fetch("/api/notes/upload", {
        body: JSON.stringify({ ...metadata, id: prepared.id, path: prepared.path }),
        headers: { "Content-Type": "application/json" },
        method: "PUT",
      });
      const completed = await completeResponse.json();
      if (!completeResponse.ok) {
        setError(completed.error ?? "Não foi possível guardar o documento.");
        return;
      }
      window.location.assign(`/notes?open=${completed.id}`);
    } catch {
          ...
          <form action={action}>
            {mode === "folder" ? (
              <>
                ...
                <div className="notes-color-options">
                  {["amber", "blue", "green", "rose", "violet", "graphite"].map((color) => (
                    <label key={color}>
                      <input
                        defaultChecked={color === "amber"}
                        name="color"
                        type="radio"
                        value={color}
                      />
                      <span className={`notes-color notes-color-${color}`} />
                    </label>
                  ))}
                </div>
              </>
            ...
            {mode === "google_doc" && googleConnected ? (
              <a className="notes-picker-link" href="/api/google/picker/start">
                Choose an existing Google Doc
              </a>
            ) : null}
...
function FolderSelect({
  ...
  return (
    <select defaultValue={selectedFolderId ?? ""} name="folder_id">
...
function NoteEditor({
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(async () => {
      const response = await fetch(`/api/notes/${item.id}`, {
        body: JSON.stringify({
          body,
          expectedGoogleRevisionId: revision,
      ...
      const payload = await response.json();
      if (!response.ok) {
        setStatus(payload.error ?? "Not saved");
        return;
      }
  ...
  return (
    <div aria-modal="true" className="notes-dialog-backdrop" role="dialog">
      <div className="notes-editor-dialog">
        <div className="notes-editor-toolbar">
          <span>
            {item.kind === "google_doc" ? "Google Note" : "OS Note"} · {status}
          </span>
          <div>
            {item.googleHtmlLink ? (
              <a href={item.googleHtmlLink} rel="noreferrer" target="_blank">
...
function DocumentFolderSelect({
  ...
  return (
    <select
      aria-label="Folder"
      className="notes-document-folder"
      disabled={disabled}
...
function NoteIcon({ item }: { item: NoteItem }) {
  return item.kind === "os_note" ? (
    <StickyNote />
  ) : item.kind === "google_doc" ? (
    <FileText />
  ) : item.mimeType?.startsWith("image/") ? (
    <FileImage />
  ) : (
...
function dialogTitle(mode: Exclude<CreateMode, null>) {
  return mode === "folder"
    ? "New folder"
    : mode === "os_note"
      ? "New OS Note"
...
function formatKind(item: NoteItem) {
  return item.kind === "os_note"
    ? "OS Note"
    : item.kind === "google_doc"
      ? "Google Note"
