...
export const createSprintModule = cache(async () => { const supabase = await createClient(); const repository = new SupabaseSprintRepository(supabase); return { service: new SprintService(repository, new SupabaseMemberDirectory(supabase), new SupabaseSprintTaskDirectory(supabase)), readModel: new SupabaseSprintReadModel(supabase) }; });
