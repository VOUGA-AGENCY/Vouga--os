...
async function fileToAvatarDataUrl(file: File): Promise<string> {
  ...
  try {
    ...
    if (dataUrl.length > MAX_AVATAR_DATA_URL_LENGTH)
      throw new Error("A imagem é demasiado pesada. Usa uma fotografia mais leve.");
    return dataUrl;
  } finally {
...
export function ContactForm({
  ...
  const cancelHref = returnTo?.startsWith("/")
    ? returnTo
    : contact
      ? `/relations/contacts/${contact.id}`
      : "/relations?view=profiles";
  ...
  return (
    <form action={formAction} className="crm-editor crm-contact-editor">
      ...
      <FormFields>
        <section className="crm-editor-group crm-editor-identity">
          <div className="crm-avatar-field">
            ...
            <div className="avatar-import-preview" aria-hidden="true">
              {avatarUrl ? (
                <Image alt="" height={80} src={avatarUrl} unoptimized width={80} />
              ) : (
                initials(contact?.displayName ?? "Perfil")
              )}
            </div>
            <div>
              <label className="crm-file-button" htmlFor="avatar_file">
                Escolher fotografia
              </label>
              <input
          ...
          <div className="crm-editor-grid">
            ...
            {prospectingContext && !contact ? (
              <input name="relationship_role" type="hidden" value="prospect" />
            ) : (
              <div className="field field-light">
                <label htmlFor="relationship_role">Relação</label>
                <select
                  defaultValue={contact?.relationshipRole ?? "prospect"}
        ...
        <section className="crm-editor-group">
          ...
          <div className="crm-editor-grid">
            {prospectingContext && !contact ? (
              ...
              <>
                <div className="field field-light">
                  <label htmlFor="company_id">Organização existente</label>
                  <select
                    defaultValue={contact?.companyId ?? defaultCompanyId ?? ""}
                    disabled={Boolean(newOrganisationName.trim())}
                    id="company_id"
                    name="company_id"
                  >
                ...
                <div className="field field-light">
                  <label htmlFor="new_company_name">Ou criar nova</label>
                  <input
                    id="new_company_name"
                    maxLength={160}
                    name="new_company_name"
            ...
            <div className="field field-light crm-span-2">
              <label htmlFor="owner_member_id">Owner no Vouga OS</label>
              <select
                defaultValue={contact?.ownerMemberId ?? defaultOwnerId ?? members[0]?.id}
                id="owner_member_id"
        ...
        <section className="crm-editor-group">
          ...
          <div className="field field-light">
            <textarea
              defaultValue={contact?.importantContext ?? ""}
              id="important_context"
              maxLength={6000}
              name="important_context"
              placeholder="Contexto que vale a pena preservar."
              rows={4}
            />
          </div>
        </section>
