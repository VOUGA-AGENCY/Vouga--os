...
export type SprintTaskItem = Readonly<{
  taskId: string;
  title: string;
...
export type SprintListItem = Readonly<{
  id: string;
  name: string;
...
export type SprintDetail = SprintListItem &
  Readonly<{
    ...
    tasks: readonly SprintTaskItem[];
    createdAt: string;
    updatedAt: string;
  }>;
export interface SprintReadModel {
  list(): Promise<SprintListItem[]>;
  listByTaskIds(taskIds: readonly string[]): Promise<SprintListItem[]>;
  findById(id: string): Promise<SprintDetail | null>;
}
