...
import type {
  ...
  SprintListItem,
  SprintReadModel,
} from "@/projections/sprints/sprint-read-model";
...
type Row = {
  ...
  sprint_tasks: Array<{
    task_id: string;
    committed_at: string;
    closure_disposition: SprintClosureDisposition | null;
    task: {
      title: string;
...
export class SupabaseSprintReadModel implements SprintReadModel {
  ...
  async list() {
    ...
    if (error) throw new Error("Não foi possível carregar as Sprints.");
  ...
  async listByTaskIds(taskIds: readonly string[]) {
    ...
    const { data: sprints, error: sprintsError } = await this.supabase
      .from("sprints")
      .select(SELECT)
      .in("id", ids)
      .order("starts_on", { ascending: false });
  ...
  async findById(id: string) {
    const { data, error } = await this.supabase
      ...
      .eq("id", id)
      .maybeSingle();
    if (error) throw new Error("Não foi possível carregar a Sprint.");
    return data ? detail(data as unknown as Row) : null;
  }
}
function list(row: Row): SprintListItem {
  const tasks = row.sprint_tasks.flatMap((item) => (item.task ? [item.task] : []));
  return {
    id: row.id,
    name: row.name,
...
function detail(row: Row): SprintDetail {
  return {
    ...list(row),
    materialRisks: row.material_risks,
    actualResult: row.actual_result,
    learning: row.learning,
    tasks: row.sprint_tasks.flatMap((item) =>
      item.task
        ? [
            {
              taskId: item.task_id,
              title: item.task.title,
              expectedResult: item.task.expected_result,
              status: item.task.status,
              ownerDisplayName: item.task.owner?.display_name ?? "Owner indisponível",
              dueAt: item.task.due_at,
              blockedReason: item.task.blocked_reason,
              blockedNextMove: item.task.blocked_next_move,
              committedAt: item.committed_at,
              closureDisposition: item.closure_disposition,
            },
          ]
        : [],
    ),
